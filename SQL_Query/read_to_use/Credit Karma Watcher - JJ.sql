		...
		when
			...
			case
				...
				-- emerging
					case
						when
							(total_trades > 0) and					-- AT01S
							(number_of_public_records < 1) and 		-- G093S
							(open_collections_accounts < 1) and		-- G215A
							(open_bankruptcy = 0) and				-- OPENBKC
							(late30 < 2) and						-- ATTR03
							(fico_score > 549) then true			-- fico_score
						else false
					end
			end
	end as rules_pass,
...
select
	...
	case
		when (age > 18) and (crd.rules_pass) and (ccms.model_score_pass) and crd.segment = 'established' then
			case
				when (crd.fico_score >= 630) and (ccms.cc_model_score <= 0.05) then 1000
				else
					case
						when (crd.fico_score >= 600) and (ccms.cc_model_score <= 0.07) then 750
						else
							case
								when (crd.fico_score >= 570) and (ccms.cc_model_score <= 0.1) then 500
								else 300
							end
					end
			end
		when (age > 18) and (crd.rules_pass) and (ccms.model_score_pass) and crd.segment = 'emerging' then
			case
				when (crd.fico_score >= 630) and (ccms.cc_model_score <= 0.07) then 500
				else 300
			end
		else null
