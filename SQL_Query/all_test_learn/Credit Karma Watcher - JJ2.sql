...
left join 
		(select 	
			distinct on (cr.customer_id) cr.customer_id,
			tsr.id,
			tsr.created_at,
		...
		from 
			credit_reports cr 
		left join
			transunion_secondary_reports tsr on cr.id = tsr.credit_report_id
		where
			tsr.created_at > '2017-11-15') mla_temp on ca.customer_id = mla_temp.customer_id
order by
	ca_uuid
),
...
/*
	step 2.2.2: get full credit history for accts selectd above
	...
	case 
		when 
			...
			case 
				-- established
				when 
					(open_satisfactory_trades >= 2) and 
					(fico_score between 350 and 850) and 
					(open_bankruptcy = 0) and 
					(late30 < 2) and 
					(fico_score > 549) then true
				-- emerging
				when 
					(open_satisfactory_trades < 2) or 
					(fico_score not between 350 and 850) and 
					(total_trades > 0) and 
...
left join 
	credit_decisions cd on ca.credit_decision_id = cd.id
left join
	product_decisions pd on cd.id = pd.credit_decision_id
where
	cd.version = 'default/credit-card/en-US/1.1'
order by
	ca_uuid, 
	customer_id
),
...
where
	ca.customer_id not in (49307425, 76729523) and 
	ca.id <> 94649950
order by
-- 	card_status, 
-- 	cc_app_stage,
